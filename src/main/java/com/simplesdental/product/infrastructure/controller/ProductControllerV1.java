...
public class ProductControllerV1 {
    ...
    public Page<ProductResponse> getAllProducts(Pageable pageable) {
        log.info("Fetching all products V1 - page: {}, size: {}", pageable.getPageNumber(), pageable.getPageSize());
        Page<ProductOutput> outputs = productUseCase.getAllProducts(pageable);
        return outputs.map(productMapper::toResponse);
    }
    ...
    @GetMapping("/{id}")
    public ProductResponse getProductByIdV1(@PathVariable Long id) {
        log.debug("Fetching product with ID: {}", id);
        var productOutput = productUseCase.getProductById(id);
        return productMapper.toResponse(productOutput);
    }
    ...
    public ProductResponse createProductV1(@Valid @RequestBody ProductRequest request) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProductResponse> updateProductV1(@PathVariable Long id, @RequestBody ProductRequest request) {
        ...
        return ResponseEntity.ok(productMapper.toResponse(response));
    }
